...
// void add_op::do_op(dtensor::tensor_type p) {
//     assert(this->inputs.size() != 0);
//     // 实现加法操作的逻辑
...
//     case tensor_type::layer:
//         new_node = new origin(inputs[0]->get_n(), inputs[0]->get_batch_num()); // origin 作为加法操作的输出层
//         new_node->set_type(tensor_type::common);
//         new_node->set_op_last(this);
//         this->output = new_node;
//         break;
...
// void sub_op::do_op(dtensor::tensor_type p) {
//     assert(this->inputs.size() != 0);
//     // 实现减法操作的逻辑
//     dtensor::dtensor_base* new_node = nullptr;
//     switch (p)
...
// add
void add_op::forward(size_t batch_id) {
...
// sub
void sub_op::forward(size_t batch_id) {
#ifdef USE_DEBUG
...
// matmul
void matmul_op::forward(size_t batch_id) {
#ifdef USE_DEBUG
    assert(this->metrix_inputs.size() == 2);
#endif
    float* otpt = this->output->get_input_data_ptr(batch_id);
...
void matmul_op::forward() {
    for(int i = 0; i < this->batch_num; ++i)
        this->forward(i);
}
// dot
void dot_op::forward(size_t batch_id) {
#ifdef USE_DEBUG
    assert(this->metrix_inputs.size() == 2);
#endif
...
// concat
void concat_op::forward(size_t batch_id) {
    // 拼接操作的前向计算逻辑
    float* otpt = this->output->get_input_data_ptr(batch_id);
...
// add
void add_op::backward(size_t batch_id) {
    float* nxt_grad = this->output->get_grad_data_ptr(batch_id); // 也可以考虑对输出作复制优化，而非每次都算一遍
    for(int i = 0; i < this->inputs.size(); ++i)
        base::_equal_tensors(this->metrix_inputs_grad[i][batch_id], nxt_grad);
}
...
// sub
void sub_op::backward(size_t batch_id) {
    float* nxt_grad = this->output->get_grad_data_ptr(batch_id); // 也可以考虑对输出作复制优化，而非每次都算一遍
#ifdef USE_DEBUG
    assert(this->metrix_inputs_grad.size() == 2);
#endif
    base::_equal_tensors(this->metrix_inputs_grad[0][batch_id], nxt_grad);
    base::_equal_neg_tensors(this->metrix_inputs_grad[1][batch_id], nxt_grad);
}
void sub_op::backward() {
    for(int i = 0; i < this->batch_num; ++i)
        this->backward(i);
}
// matmul
void matmul_op::backward(size_t batch_id) {  //还需仔细检查！！
...
void matmul_op::backward() {
    for(int i = 0; i < this->batch_num; ++i)
        this->backward(i);
}
// dot
void dot_op::backward(size_t batch_id) {
...
void dot_op::backward() {
    for(int i = 0; i < this->batch_num; ++i)
        this->backward(i);
}
// concat
void concat_op::backward(size_t batch_id) {
    // 拼接操作的反向计算逻辑
}
