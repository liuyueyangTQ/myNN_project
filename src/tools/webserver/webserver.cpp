...
std::string httpServer::readLocalWebFile(std::string relativePath) { // 读取本地html文件
...
std::string httpServer::httpResponse(int code, const std::string &text, const std::string &body,
...
std::string httpServer::apiPageJson(int n) {
    if (n < 1 || n > total_pages) n = 1;
...
// ========== File I/O ==========
std::vector<std::string> httpServer::readLines(const std::string &path) {
...
bool httpServer::appendLine(const std::string &path, const std::string &line) {
...
bool httpServer::writeFile(const std::string &path, const std::string &content) {
...
void httpServer::ensureDataDir() { _mkdir(std::string(DATA_DIR).c_str()); }
// ========== User System ==========
std::string httpServer::generateUid() {
...
std::string httpServer::userFilePath(const std::string &uid) {
    return this->basic_path + DATA_DIR + "/user_" + uid + ".txt";
}

std::string httpServer::getUserName(const std::string &uid) {
    auto lines = readLines(userFilePath(uid));
    if (!lines.empty()) return lines[0];
    return "\xe8\xae\xbf\xe5\xae\xa2";
}

void httpServer::setUserName(const std::string &uid, const std::string &name) {
    auto lines = readLines(userFilePath(uid));
    std::string out = name + "\n";
...
std::vector<std::string> httpServer::getUserHistory(const std::string &uid) {
    auto lines = readLines(userFilePath(uid));
    if (lines.size() <= 1) return {};
    return std::vector<std::string>(lines.begin() + 1, lines.end());
}

void httpServer::addUserHistory(const std::string &uid, int p, const std::string &ts, const std::string &cm) {
...
// ========== Page Metadata ==========
std::string httpServer::pageTitle(int n) {
...
std::string httpServer::pageIcon(int n) {
...
std::string httpServer::pageShortLabel(int n) {
...
std::string httpServer::commentFile(int page) {
...
std::string httpServer::resolveUser(const std::string &cookieHeader, std::string &outUid, std::string &outSetCookie) {
...
std::string httpServer::contentHTML(int n) {
    // 返回页面内容的 HTML，在httpServer初始化的时候已经存入localPageContents数组中
    if (n >= 1 && n <= total_pages) return localPageContents[n - 1];
    return "";
}

void httpServer::invalidateCache(int page) {
    if (page >= 1 && page <= total_pages) 
        pageCaches[page - 1].valid = false;
}

// Build JSON for user info: uid, username, history[]
std::string httpServer::apiUserJson(const std::string &uid) {
...
std::string httpServer::getCookie(const std::string &h, const std::string &name) {
    std::string s = name + "=";
    size_t p = h.find(s);
...
int httpServer::extractTrailingNum(const std::string &path) {
